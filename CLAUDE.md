...
| `frontend/btn-fill.js` | Cursor-origin expanding-fill interaction for CTA/action buttons (`.btn` family in the dashboard, `.btn-nav`/`.btn-primary-lg`/`.btn-ghost-lg` on the landing pages). Delegated `pointerover`/`pointermove`/`pointerout`/`focusin`/`focusout` listeners on `document` set `--fill-x`/`--fill-y`/`--fill-d` and toggle `.is-filling`; the actual fill circle and color fade are CSS (`styles.css` for the app, each landing page's own `<style>` block). See "Cursor-Origin Expanding-Fill Buttons" below. |
...
## Cursor-Origin Expanding-Fill Buttons (implemented)

Replaces the hover/click treatment on the app's CTA/action buttons with a cursor-origin expanding-fill interaction: resting state is a transparent, accent-tinted surface with a 1px accent border and accent-colored text; on hover a dark-ink circle expands via `transform: scale()` from the pointer's entry point (or the button's center, for keyboard focus and touch) to cover the button in ~500ms, with the text/icon brightening for contrast. Design discussion: 2026-09-16.

Key decisions:

- **Scope is CTA/action buttons only, not every clickable control.** Covers the `.btn` family in the dashboard (Login, Change Password, Run Simulation, Reset, Ask, Download CSV/PDF/Summary, Save Simulation, Back to Dashboard, modal Cancel/Confirm, the error-retry button) plus the landing pages' `.btn-nav`/`.btn-primary-lg`/`.btn-ghost-lg` CTAs. Deliberately excluded: icon-only controls (theme toggle, user avatar, agent FAB, sidebar rail icons), tabs (`result-tab`), accordion headers (`sidebar-group-header`), and menu rows (`user-menu-item`) — the radial-fill + text-color-fade pattern is built for text/pill CTAs and would look inconsistent applied to those other control shapes.
- **Resting colors changed from solid brand fill to transparent-tinted + bordered**, so the fill animation has visible "room" to expand into (the original prompt's spec — transparent surface, accent border, accent text at rest — genuinely doesn't work if the button is already a solid color). `btn-primary`/`btn-teal` moved from solid `var(--teal)` backgrounds to `rgba(var(--teal-rgb),0.10)` + teal border/text; `btn-outline`/`btn-secondary` were already close to this shape and only needed the border tightened for visibility. The accent palette itself (teal/gold) is unchanged — same tokens, just used as border/text instead of background at rest.
- **The fill circle is a fixed dark ink (`#07091A`) in both light and dark theme**, matching the pre-existing convention of hardcoding this exact value for on-accent text elsewhere in `styles.css`. Because of that, **the filled-state text/icon color must also be a fixed bright color, not a theme-reactive token** — this was a real bug found post-ship: `.btn-outline.is-filling`/`.btn-secondary.is-filling` originally used `var(--text)`/`var(--gold-lt)`, which flip to *dark* values in light theme (correct for light-on-white text, wrong against this always-dark fill), producing unreadable dark-on-dark text on hover/focus in light mode. Fixed to hardcoded `#F0F4FF` / `#E8A04A` (the dark-theme values), same pattern already used correctly for `btn-primary`/`btn-teal`'s `#0ee8c4`.
- **Diameter is computed per-button in JS, not a fixed guess.** `btn-fill.js` sets `--fill-d` to `hypot(width, height) * 2` on each pointerover/focus (reusing the `getBoundingClientRect()` already needed for the origin), so the circle always fully covers the button regardless of size — from small `btn-sm` pills to full-width `btn-block` and the large landing-page hero CTAs — without per-variant tuning.
- **Stacking is solved with `isolation: isolate` + `z-index: -1` on the `::after`, not by wrapping every button's text in a positioned `<span>`.** A negative-z-index absolutely-positioned pseudo-element inside a `.btn` that establishes its own stacking context (via `isolation: isolate`) paints behind all of that button's normal in-flow content automatically — so bare text nodes (e.g. the landing pages' `.btn-nav`/`.btn-ghost-lg`, which have no wrapping span) don't need markup changes for the fill to render behind them.
- **Listeners are delegated on `document` in `btn-fill.js`, not attached per-button.** Covers buttons injected after load (e.g. the error-retry button in `app.js`'s `renderError()`) for free, and there is nothing to wire up or tear down.
- **Emoji/Unicode icons inside these buttons were replaced with inline `currentColor` SVGs** (`aria-hidden`), so they fade in sync with the text color transition and render consistently across devices: the reset (↺), download (⬇, reused from the existing Report sidebar-group icon), save (💾), back (←), and landing-page nav/ghost arrows (→ ↓). `lang.js`'s translation strings for these buttons (`btn.reset`, `btn.download*`, `btn.save.simulation`, `nav.back`) had their emoji prefixes stripped accordingly, in both BM and EN. Two spots in `app.js` (`renderResults()` and `submitSaveSimulation()`) that used to set the Save button's entire `textContent` were changed to update only the `.btn-text` span and swap the icon's `innerHTML`, since overwriting the whole button would have wiped the icon.
- **`prefers-reduced-motion: reduce` removes the spatial scale animation**, showing/hiding the already-full-size fill circle via a short `opacity` transition instead, per the original prompt's requirement.
